...
const Navbar = () => {
  ...
  useEffect(() => {
    setUserAuthenticated(
      userDetails.email !== "" || sessionStorage.getItem("email") !== null
    );
  }, [userDetails]);
  ...
  return (
    <nav className={styles.navbar}>
      <div className={styles.navbar__brandName}>
        <h3>
          <Link exact to={userDetails.email === "" ? "/" : "/welcome"}>
            ASSURENEWS
          </Link>
        </h3>
      </div>
      ...
      <div
        style={{ display: `${userAuthenticated ? "none" : ""}` }}
      ...
      >
        ...
        {!userAuthenticated ? (
          <div
            className={styles.navbar__mobileLinkAction}
